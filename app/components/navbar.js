...
const Navbar = ({ user, onToggleSidebar, leads = [], setActiveTab }) => {
  ...
  const filteredLeads = (searchTerm.trim() === '') ? [] : (leads || []).filter(lead => {
  ...
  const navOptions = [
    ...
    ...(user?.role === 'admin' ? [{ id: 'users', label: 'User Authorization & Management' }] : []),
    { id: 'settings', label: 'Settings' },
  ];

  const filteredNav = (searchTerm.trim() === '') ? [] : navOptions.filter(item =>
