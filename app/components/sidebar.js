...
const Sidebar = ({
  ...
  return (
    <aside
      className={`${isCollapsed ? "w-16" : "w-64"} bg-white border-r border-slate-200/80 flex flex-col justify-between h-[calc(100vh-64px)] sticky top-16 select-none shrink-0 py-4 transition-all duration-300 overflow-hidden`}
    >
            ...
            {companyItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center ${isCollapsed ? "justify-center px-2 py-2.5" : "justify-between px-3 py-2.5"} rounded text-xs font-semibold tracking-wide transition-all text-left cursor-pointer group ${
                    isActive
