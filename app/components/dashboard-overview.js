...
import { Wallet, CreditCard, DollarSign, Users, TrendingUp } from 'lucide-react';
...
export default function DashboardOverview({
  ...
  const getTrendData = () => {
    if (trendPeriod === 'weekly') {
      ...
      return dayOrder.map(day => ({
        label: day,
        count: counts[day],
        pct: counts[day] === 0 ? '8%' : `${Math.max(12, Math.round((counts[day] / maxCount) * 100))}%`
      }));
      ...
      return months.map((month, idx) => ({
        label: month,
        count: counts[idx],
        pct: counts[idx] === 0 ? '8%' : `${Math.max(12, Math.round((counts[idx] / maxCount) * 100))}%`
      }));
    } else {
      ...
      return Object.keys(quarters).map(q => ({
        label: q,
        count: quarters[q],
        pct: quarters[q] === 0 ? '8%' : `${Math.max(12, Math.round((quarters[q] / maxCount) * 100))}%`
      }));
    }
  };
  ...
  const totalPaidSum = leads.reduce((sum, l) => sum + (Number(l.paidAmount) || 0), 0);
  const totalBalanceSum = leads.reduce((sum, l) => sum + (Number(l.balanceAmount) || 0), 0);
  const totalDealSum = leads.reduce((sum, l) => sum + (Number(l.totalAmount) || ((Number(l.paidAmount) || 0) + (Number(l.balanceAmount) || 0))), 0);
    ...
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white p-5 rounded-2xl shadow-sm flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-xl bg-white/10 backdrop-blur-xs flex items-center justify-center text-white shrink-0">
        ...
        <div className="bg-white border border-slate-200/80 rounded-xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between h-[135px]">
          <div className="flex justify-between items-start">
            <div>
              ...
              <span className="text-2xl font-extrabold text-slate-800 mt-1 block">{leads.length.toLocaleString()}</span>
            </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-xl p-5 shadow-sm hover:shadow-md transition-all flex items-center justify-between h-[135px]">
          ...
          <div className="relative w-16 h-16 flex items-center justify-center shrink-0">
            <svg className="w-full h-full transform -rotate-90">
              ...
              <circle cx="32" cy="32" r="26" stroke="#0ea5e9" strokeWidth="6" fill="transparent" strokeDasharray="163.3" strokeDashoffset={0} strokeLinecap="round" />
            </svg>
            <span className="absolute text-xs font-bold text-slate-700">{systemUsersCount}</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between h-[135px]">
          <div className="flex justify-between items-start">
            <div>
              ...
              <span className="text-2xl font-extrabold text-slate-800 mt-1 block">₹{totalPaidSum.toLocaleString('en-IN')}</span>
            </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-[360px] relative overflow-hidden group">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-extrabold text-slate-800 text-sm tracking-tight">Prospect Engagement Trends</h3>
                  <span className="text-[10px] font-bold bg-sky-50 text-sky-600 border border-sky-200/60 px-2 py-0.5 rounded-full">
                    {currentTrendBars.reduce((sum, b) => sum + b.count, 0)} Total
                  </span>
                </div>
                <p className="text-[11px] text-slate-400 font-medium mt-0.5">
                  {trendPeriod === 'weekly' && 'Daily prospect engagements (Mon – Sun)'}
                  {trendPeriod === 'monthly' && 'Full 12-month engagement report (Jan – Dec)'}
                  {trendPeriod === 'yearly' && 'Quarterly engagement distribution (Q1 – Q4)'}
                </p>
              </div>
          ...
          <div className="relative flex-1 flex items-end justify-between gap-1.5 sm:gap-2 px-1 pt-6 pb-1 mt-2">
            ...
            {currentTrendBars.map((bar) => (
              <div key={bar.label} className="flex-1 flex flex-col items-center group/bar h-full justify-end relative z-10">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-[360px] relative overflow-hidden group">
          ...
          <div className="h-40 w-full flex items-end relative my-2">
            <svg className="w-full h-full overflow-visible" viewBox="0 0 200 100" preserveAspectRatio="none">
              ...
              <circle cx="10" cy={y1} r="4" fill="#10b981" stroke="#ffffff" strokeWidth="2" />
              <circle cx="75" cy={y2} r="4" fill="#10b981" stroke="#ffffff" strokeWidth="2" />
              <circle cx="140" cy={y3} r="4" fill="#10b981" stroke="#ffffff" strokeWidth="2" />
              <circle cx="190" cy={y4} r="4" fill="#10b981" stroke="#ffffff" strokeWidth="2" />
            </svg>
          </div>
          ...
          <div className="grid grid-cols-4 gap-1 text-center pt-2 border-t border-slate-100">
            <div>
              <span className="text-[10px] font-bold text-slate-400 block uppercase">Q1</span>
              <span className="text-xs font-extrabold text-slate-800 font-mono">₹{q1.toLocaleString('en-IN')}</span>
            </div>
            <div>
              <span className="text-[10px] font-bold text-slate-400 block uppercase">Q2</span>
              <span className="text-xs font-extrabold text-slate-800 font-mono">₹{q2.toLocaleString('en-IN')}</span>
            </div>
            <div>
              <span className="text-[10px] font-bold text-slate-400 block uppercase">Q3</span>
              <span className="text-xs font-extrabold text-slate-800 font-mono">₹{q3.toLocaleString('en-IN')}</span>
            </div>
            <div>
              <span className="text-[10px] font-bold text-slate-400 block uppercase">Q4</span>
              <span className="text-xs font-extrabold text-slate-800 font-mono">₹{q4.toLocaleString('en-IN')}</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="overflow-x-auto">
              {leads.length > 0 ? (
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-100 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">
                      ...
                      <th className="pb-3 text-right">Financials (Paid / Bal)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {leads.slice(0, 5).map((lead) => (
                      <tr key={lead.id || lead._id} className="border-b border-slate-100/60 last:border-0 hover:bg-slate-50/50 transition-colors font-semibold text-xs text-slate-600">
                        ...
                        <td className="py-3 pr-2">
                          <div className="flex flex-col">
                            <span className="text-slate-700 font-bold">{lead.areaZone || 'General'}</span>
                            <span className="text-[10px] text-slate-400">{lead.campaign || 'Direct Outbound'}</span>
                          </div>
                        </td>
                        <td className="py-3 text-right">
                          <div className="flex flex-col items-end font-mono">
                            <span className="text-emerald-600 font-bold text-xs">₹{(Number(lead.paidAmount) || 0).toLocaleString('en-IN')}</span>
                            {(Number(lead.balanceAmount) || 0) > 0 && (
                              <span className="text-[10px] text-amber-600 font-bold">Bal: ₹{(Number(lead.balanceAmount) || 0).toLocaleString('en-IN')}</span>
                            )}
                          </div>
                        </td>
                      </tr>
                ...
                <div className="text-center py-10 flex flex-col items-center justify-center">
                  <span className="text-sm font-bold text-slate-700">No Backend Leads Found</span>
                  <p className="text-xs text-slate-400 font-medium mt-1">Start by adding your first lead to generate dynamic calling analytics.</p>
                  <button
                    onClick={() => setActiveTab('leads')}
        ...
        <div className="bg-white border border-slate-200/80 rounded-xl p-6 shadow-sm overflow-hidden flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-slate-800 text-sm mb-1">Active Geographies & Zones</h3>
            <p className="text-xs text-slate-400 font-semibold mb-6">Backend lead distribution by zone</p>
                ...
                <div key={idx} className="flex flex-col gap-1.5">
                  <div className="flex justify-between items-center text-xs font-semibold text-slate-700">
                    ...
                    <span className="text-slate-500 text-[11px] font-mono">
                      {item.calls} {item.calls === 1 ? 'lead' : 'leads'} ({item.pct}%)
                    </span>
                  </div>
                  <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                    <div className={`${item.color} h-full transition-all duration-500`} style={{ width: `${Math.max(4, item.pct)}%` }} />
                  </div>
                </div>
              ))}
            </div>
